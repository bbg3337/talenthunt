...
public interface AssessmentRepository extends JpaRepository<Assesment, Long> {
	...
	public static final String candidateAssementBaseOnToken="SELECT * FROM a2t_assessments ass "
			+ "WHERE ass.assessment_id in "
			+ "(select candidate.assessment_id from a2t_company_candidates candidate "
			+ "where candidate.access_code = :accessCode)";
	...
	@Query(value = candidateAssementBaseOnToken, nativeQuery = true)
	public List<Assesment> getCandidatesAssessmentBaseOnToken(@Param("accessCode")String accessCode);
